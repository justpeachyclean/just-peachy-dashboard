  ...
  try {
    ...
    for (const { name_key } of groups) {
      const rows = db.prepare(`
        SELECT id, client_name, frequency, price_per_clean, quote_amount,
               initial_clean_price, annual_value, lead_source, notes, rep_name,
               converted, recurring_retained, initial_clean_booked,
               (COALESCE(converted,0)*10 + COALESCE(recurring_retained,0)*8 +
                ...
                (CASE WHEN notes IS NOT NULL AND notes!='' THEN 1 ELSE 0 END)) AS score
        FROM lead_records
      ...
      // Merge missing fields from losers into winner using COALESCE
      db.prepare(`
        UPDATE lead_records SET
          frequency         = COALESCE(frequency, ?),
          price_per_clean   = COALESCE(price_per_clean, ?),
          quote_amount      = COALESCE(quote_amount, ?),
          initial_clean_price = COALESCE(initial_clean_price, ?),
          annual_value      = COALESCE(annual_value, ?),
          lead_source       = COALESCE(lead_source, ?),
          notes             = COALESCE(notes, ?)
        WHERE id = ?
      `).run(
        losers.map(l => l.frequency).find(Boolean) ?? null,
        losers.map(l => l.price_per_clean).find(Boolean) ?? null,
        losers.map(l => l.quote_amount).find(Boolean) ?? null,
        losers.map(l => l.initial_clean_price).find(Boolean) ?? null,
        losers.map(l => l.annual_value).find(Boolean) ?? null,
        losers.map(l => l.lead_source).find(Boolean) ?? null,
        losers.map(l => l.notes).find(Boolean) ?? null,
        winner.id
      )
      ...
      for (const loser of losers) {
        ...
        deletedCount++
        log.push({ kept: winner.id, deleted: loser.id, name: winner.client_name })
      }
    }

    res.json({ ok: true, groups_cleaned: groups.length, records_deleted: deletedCount, log })
  } catch (err) {
